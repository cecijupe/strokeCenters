...
hollaApp.controller('strokeCentersController', function(socket){
  socket.on('thereWasRequest', function(data){
    console.log("there was request");
    console.log(data);
    _this.embSocket = data.socketID;
    }
    );
  socket.on('hospitalResponse', function(data){
    console.log("there was hospital response");
    console.log(data);
    }
    );
  var _this = this;
  this.sendRequest = function(){
    console.log("sending request", _this.request);
    var request = { string: _this.request};
    socket.emit("requestSent", request);
    _this.request = "";
  }
  this.responseRequest = function(){
    console.log("sending response for request", _this.response);
    var response = {
      string : _this.response,
      socketID : _this.embSocket
    }
